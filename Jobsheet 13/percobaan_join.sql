-- Active: 1713956067589@@localhost@3306@percobaan_join
...
FROM
    kelas, prodi
WHERE
    kelas.kode_prodi = prodi.kode_prodi;
...
FROM
    dosen d
    INNER JOIN jadwal j ON d.kode_dosen = j.kode_dosen
...
SELECT
    ...
    'Selasa' AS 'Hari Dipakai',
    jp_mulai.jp_mulai AS 'Dari Jam',
    jp_selesai.jp_selesai AS 'Sampai Jam'
FROM
    ruang r
    INNER JOIN jadwal j ON r.kode_ruang = j.kode_ruang
    INNER JOIN hari h ON j.kode_hari = h.kode_hari
    INNER JOIN jp jp_mulai ON j.jp_mulai = jp_mulai.kode_jp
    INNER JOIN jp jp_selesai ON j.jp_selesai = jp_selesai.kode_jp
WHERE
    h.nama_hari = 'Selasa';
...
FROM 
    dosen d
RIGHT JOIN 
    jadwal j ON d.kode_dosen = j.kode_dosen
RIGHT JOIN 
    mk ON j.kode_mk = mk.kode_mk
WHERE 
    j.kode_jadwal IS NULL OR j.kode_jadwal IS NOT NULL;
